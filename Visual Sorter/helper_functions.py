...
def get_lengths(count: int) -> list[int]:
	...
	random_numbers = [random.randint(1,75) for i in range(count)]
...
def get_rectangles(random_numbers: list[int]) -> list[int]:
	"""
	...
	rectangle_coordinates = [[100 + (length) * 35, 0, 35, random_numbers[length] * 10] for length in range(len(random_numbers))]
